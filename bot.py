...
# Создаём красивую последовательность цветов
def generate_roulette_line(frame_num, green_pos=None):
    """Генерирует линию рулетки с чередованием красный/чёрный"""
    if frame_num % 2 == 1:  # Нечётный кадр - 8 квадратов
        length = 8
    else:  # Чётный кадр - 9 квадратов
        length = 9
    
    # Чередуем цвета: начинаем с красного или чёрного
    ...
    # Вставляем зелёный в случайное место
    ...
    # Для чётных кадров добавляем 2 пробела в начало
    if frame_num % 2 == 0:
        line = '  ' + line
    
    return line, green_pos
...
class JokerGame:
    def __init__(self, uid, bet):
        ...
        self.bet = bet
        self.rows = []
        self.joker_positions = []  # Сохраняем позиции джокеров
        self.add_row()
    ...
    def add_row(self):
        ...
        for i in range(3):
            if random.random() < 0.333:
                row.append('🃏')
                joker_in_row.append(True)
            else:
                row.append('🎴')
                joker_in_row.append(False)
        self.rows.append(row)
        self.joker_positions.append(joker_in_row)
    
    def reveal(self, r, c):
        if r >= len(self.rows):
            return None
        if self.rows[r][c] == '🃏' or self.rows[r][c] == '🎴':
            is_joker = (self.rows[r][c] == '🃏')
            self.rows[r][c] = '💎' if is_joker else '❌'
            if is_joker:
                self.add_row()
                return 'win'
            return 'lose'
    ...
    def get_total_win(self):
        jokers = 0
        for row in self.joker_positions:
            for is_joker in row:
                if is_joker:
                    jokers += 1
        if jokers == 0:
...
async def show_joker_board(msg, game, is_edit=True, callback=None):
    ...
    jokers = 0
    for row in game.joker_positions:
        for is_joker in row:
            if is_joker:
                jokers += 1
...
async def roulette_animation(message, bet, bet_type, bet_value):
    # Определяем финальный результат ДО анимации
    final_color = roulette_spin()
    
    # Генерируем кадры анимации (всегда нечётное количество)
    frames = []
    green_positions = []
    
    for i in range(41):  # 41 кадр (нечётное число)
        if i % 2 == 1:  # Нечётный кадр - 8 квадратов
            line, green_pos = generate_roulette_line(i, None)
        else:  # Чётный кадр - 9 квадратов с пробелами
            line, green_pos = generate_roulette_line(i, None)
        frames.append(line)
        green_positions.append(green_pos)
    
    # Финальный кадр (всегда нечётный, 8 квадратов) с фиксированным результатом
    final_line, final_green = generate_roulette_line(1, None)
    ...
    # Финальный результат с правильным цветом
    if final_color == 'red':
        color_emoji = '🔴'
        color_text = 'RED'
        # На финальном кадре должен быть красный цвет на позиции зелёного
