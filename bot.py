...
def get_random_line():
    # Генерируем линию с 8 или 9 квадратиками для динамики
    num_slots = random.choice([7, 8, 9, 8, 7, 8, 9])
    colors = []
    for _ in range(num_slots):
        colors.append(random.choice(['⬛', '🟥']))
    # Добавляем зелёный в случайное место
    green_pos = random.randint(0, num_slots - 1)
    colors[green_pos] = '🟩'
    return ''.join(colors)
...
# ========== РУЛЕТКА С ДИНАМИЧЕСКОЙ АНИМАЦИЕЙ ==========
async def roulette_animation(message, bet, bet_type, bet_value):
    # Генерируем кадры с разным количеством элементов
    frames = []
    for _ in range(45):
    ...
    for i, frame in enumerate(frames):
        ...
        # Добавляем визуальный эффект - меняем количество квадратиков
        bar = "=" * random.randint(6, 12)
        new_text = (
            f"🎡 КРУТИМ РУЛЕТКУ\n\n"
            f"{bar}|{bar}\n"
            f"{frame}\n"
